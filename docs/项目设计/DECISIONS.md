...
## ADR-22 · 2026-06-24 · 事实接地：order_items 表 + 订单查询主动列单（落 ADR-21 的 P2）
**决策**：补 `order_items` 表（商品名/数量/单价）+ `get_order_items` 工具；`OrderQuerySkill` 两处改造：① `order_id` 未知时**主动列出本人近期订单**（不再卡"请提供订单号"）② 订单详情附**商品明细**。意图词补 "买了什么/买了啥/买过什么" 入 `order_query`。明细一律**查库**填，不让 LLM 编。
**为什么**：LLM-Judge 实测 order_query helpfulness 垫底；根因是"买了什么"无数据源（无 order_items）+ 无单号就卡死。事实接地直接拉 helpfulness/accuracy，且延续读写分离（条目级事实查库不臆测）。
**否决**：让 LLM 从对话"推断"买了什么（幻觉）；无单号继续卡死（体验差、解决率低）。

## ADR-23 · 2026-06-24 · 评测多轮校准：turns 同 ticket 顺序跑 + 按对象数硬验去重
**决策**：`run_eval` 支持多轮 case（`turns` 列表，**同一 ticket 顺序跑**，runner 加载历史+续接状态），**期望字段缺省即不断言**（某些轮只关心 state）；新增 `expected_refund_count` **按 order_id 数退款单**，把 ADR-19 会话级去重纳入可量化评测。补确认流/取消/跨轮去重/迟疑续接 4 条多轮 + 提示注入/越权查询 2 条对抗。
**为什么**：旧评测纯单轮，**测不了确认流/去重/续接**——尺子歪的，过了也不代表多轮鲁棒。校准尺子本身是交付，且当场暴露 bug（去重计数误按 user_id 跨 case 串，改按 order_id 隔离）。
**否决**：只单轮评测（多轮路径无覆盖）；去重只靠单测不入评测（无量化回归）。

## ADR-24 · 2026-06-24 · 接入层限流：Redis 固定窗口 + fail-open
**决策**：`POST /chat/message` 入队前按 `user_id` 限流（`app/core/ratelimit`，`INCR+EXPIRE` 固定窗口），超额 429；配额 `chat_rate_limit_per_min` 配置化（<=0 关闭）。**Redis 异常 → fail-open 放行**。
**为什么**：异步架构里 POST 只入队即返回，突发流量会灌爆队列、打穿下游 LLM 成本——限流是接入层削峰前置闸，挡在入队前。固定窗口简单原子够用；限流是流控非正确性闸，Redis 抖动时宁可放过也不误杀正常流量（同 ADR-4 Redis 只担非正确性职责）。
**否决**：滑动窗口（MVP 不需要那复杂度）；fail-closed（Redis 挂了把正常请求也拦死，可用性更差）。

## ADR-25 · 2026-06-24 · Provider 熔断降级：包装 LLMClient 的三态熔断
**决策**：`CircuitBreakerLLMClient` 包住任意适配器（对外仍是 `LLMClient`，agent loop 无感），三态熔断（closed/open/half_open）：连续失败达阈值→熔断，冷却内**快速失败**或**切 `llm_fallback_model` 无缝降级**，冷却后半开试探。`registry` 默认套熔断（拆 `_build_raw` 供主/备复用），可配开关。
**为什么**：LLM 是链路里最慢、最易抖、且是单点的外部依赖；provider 故障时若每个请求硬调+超时+重试会把 worker 池堵死（雪崩）。熔断在连续失败后快速失败/降级，不再往死掉的 provider 灌。**正是 ADR-7/11 provider 抽象的价值兑现**——抽象在，才能包熔断、配 fallback。
**否决**：不熔断（provider 抖动→雪崩）；在 agent_core 里写熔断（散落、耦合，不如包在 LLMClient 层组合）。

## ADR-26 · 2026-06-24 · SSE 流式：服务端推送进度替代轮询（非 token 流式）
**决策**：`GET /chat/session/{id}/stream` 返回 `text/event-stream`，服务端轮询会话视图、**状态/步骤/回复有变化才推 `data`**、终态推 `done` 收尾（`max_iters` 兜底）。前端 ChatView 用 `EventSource` 接，`done` 主动关闭防自动重连，**异常/mock/不支持 → 退回轮询**。
**为什么**：轮询体验有延迟感；SSE 把"客户端拉"换成"服务端推"，进度实时点亮。**诚实定位**：回复由确定性 `finalize` 生成，不是 LLM 逐 token，所以推的是**处理进度**而非 token 流式——这是本架构下真实的流式。每轮 `expire` 缓存重读才看得到 worker 进程的提交。
**否决**：谎称 token 级流式（架构对不上）；WebSocket（SSE 单向推送够用、更简单）；Redis pub/sub（worker 跨进程，MVP 用服务端轮询转推更省事）。

## ADR-27 · 2026-06-24 · 长对话记忆=滚动摘要+token预算，不用向量记忆（落 ADR-21 的 P4）
**决策**：`_load_history` 按 **token 预算**切：最近若干条原样保留，**超预算的旧段 LLM 摘成一条前置消息**（`app/agent/memory`，模板兜底、stub/CI 可复现）；`history_token_budget` 配置化。**客服记忆不上向量库。**
**为什么**：客服重要信息**几乎全是结构化**（订单/退款/物流/政策）→ 该**查库**不该向量"回忆"（ADR-21）；向量长期记忆只在**非结构化上下文多**的领域（心理咨询/研究 Agent）才值，客服里上它=凑 RAG（召回噪声+幻觉）。滚动摘要**一套机械同时解决两件事**：长对话不忘最初诉求（P4 记忆）+ 不超上下文窗口（TODO-2 溢出预防）。**硬键（order_id/金额）不靠摘要**——在 ticket/DB 查库，摘要只扛软上下文（经过/诉求/语气），即便有损也丢不了关键键。
**否决**：向量长期记忆（客服场景凑 RAG，价值低、引幻觉）；按条数截断（会丢"最初诉求"）；不处理（长对话超窗口直接硬崩 failed）。
**待办（可选 backstop）**：LLM 真报上下文超限时"截断重试一次→仍不行转人工"的软着陆。token 预算已从源头预防正常长对话溢出，此为极端兜底，价值低、需 provider 特定错误识别，列为可选。

## ADR-28 · 2026-06-24 · SLA 监控闭环：解决回填 + 达成统计进 metrics
**决策**：补全 `sla_records` 的闭环——此前建工单只写 `deadline`，`is_timeout`/`resolved_at` **从不更新**。现：工单 `resolved_by_agent` 时 `sla_service.mark_resolved` 回填 `resolved_at` + 判 `is_timeout`；`sla_stats` 汇总 met/breached/pending/met_rate（breached 含"未解决已过期"=正在违约）；经 `compute_metrics` + `AdminMetrics.sla` 透到 `/admin/metrics`。
**为什么**：SLA 是客服的核心运维指标（承诺时效达没达标），但原来只埋数据不闭环=半截功能。补回填+统计是诚实的真改进（不是凑功能）。转人工/等待态暂不回填——SLA 解析口径=Agent 自助解决；升级人工的工单 SLA 继续计时，过期即计入 breached（符合真实运维语义）。
**否决**：继续只写不查（数据无用）；用后台定时任务扫超时（MVP 在解决点回填 + 查询时算"正在违约"已够，不引调度）。
