...
> 真值优先级:代码 + 测试 > 本文。配套:`定位与亮点.md`(讲法)/`DECISIONS.md`(为什么)/`01-总体设计.md`(契约)。

**章节进度**（逐步生成）：
- [x] 0. 定位 + 整体架构
- [x] 1. 一条消息的完整生命周期 ★主脊
- [x] 2. 意图识别　[x] 3. 路由　[x] 4. Agent 执行管道　[x] 5. Skill
- [x] 6. 工具调用　[x] 7. 退款确认流　[ ] 8. 记忆　[ ] 9. 评测
- [ ] 10. 前端两端　[ ] 11. 数据模型　[ ] 12. 设计哲学串联
...
flowchart TD
  ...
  subgraph INF[工具 / 服务 / 数据]
    T[Tools 工具层 + 自动审计]
    S[Services 业务逻辑]
    DB[(sqlite 开发 / PostgreSQL 生产<br/>11 张表)]
...
**分层职责**：
- **前端**：用户端提问看回复，运维端看指标/工单/会话。
- **API**：`POST /chat/message` 只落库+入队**立即返回**(不阻塞)；`GET /chat/session/{id}` 轮询结果。
...
先看全局：用户发一句话，到拿到回复，经历了什么。**关键是异步——发消息不阻塞，结果靠轮询。**
...
sequenceDiagram
  ...
  loop ⑪ 轮询直到 task_status 终态
    U->>API: GET /chat/session/{id}
    API->>DB: 读会话视图
    API-->>U: {taskStatus, latestReply, steps 时间线, toolCalls, tokenUsage}
  end
```
...
- **⑪ 轮询闭环**：前端拿 `taskStatus`(queued→processing→final/need_human)，顺带拿"思考过程时间线 + 工具调用审计 + token 成本"——可观测的来源。
...
class RefundHandlingSkill:
    ...
    def finalize(self, ctx, tool_ctx):                     # loop 之后:确定性决策 + 写
        ...
        ticket.pending_action = {"type": "refund_request", "order_id": ctx.order_id, ...}
        return Decision("确认要退款 X 元吗？回复确认/取消。", States.WAITING_USER_CONFIRM)
```
...
## 7. 退款确认流(两轮 + 状态机 + pending_action)

高危退款不一句话就退,而是**两轮**:第 1 轮存"待执行动作"并转待确认态,第 2 轮"确认"才真正建草稿。靠 `pending_action`(存工单)跨轮 + 状态机续接。
...
sequenceDiagram
  ...
  A->>T: 存 pending_action={refund_request, order_id} + 工单状态=waiting_user_confirm
  A-->>U: 「确认要退款 120 元吗?回复 确认/取消」
  Note over U,T: 下一轮 runner 读到工单状态=waiting_user_confirm → 续接确认态
  U->>A: 轮2「确认」(同一工单)
...
# finalize 里的确认分支:绑定校验 + 幂等 + 建草稿
def _confirm(self, db, ctx, ticket):
    pa = ticket.pending_action
    if not pa or pa["type"] != "refund_request":   # 防跨业务误确认
        return Decision("当前没有待确认的退款。", States.RESOLVED_BY_AGENT)
...
**为什么这么设计**:
- **二次确认门**:高危写不一句话就执行,先确认——防误退(层5)。
- **pending_action 绑定具体动作**:不只看"在等待确认态",还绑了"确认的是哪个退款"——防"确认"被跨业务误触发。
...
转人工那一刻,把上下文打包给人工客服(`handoff_service`,TODO-1)——**事实查库、叙述靠 LLM**:
...
> 衔接:这一节把"业务正确"升级到"工程可用"——正确性在前 9 节,可用性(起得来/看得见/崩不动)在这一节。下一步可补:并发实测数据(ADR-14 的 PG 真并发)与降级的边界(上下文溢出 TODO-2)。
