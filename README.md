...
## 核心能力
- **意图识别**：规则快路 + LLM 兜底
- **Skill 业务流程**：RefundHandling（低风险自动退款草稿 / 高风险转人工）、LogisticsException（催件 / 签收未收到转人工）、General 兜底（投诉·要求人工 → 转人工）
...
- **异步闭环**：API 落库+入队立即返回，worker 异步消费，轮询取结果（含「思考过程」时间线）
- **可观测**：agent_tool_calls 审计、token/cost 记账、运行时 metrics
- **评测 + guardrails 对抗**：30 常规 + 5 对抗样例，量化合规拦截率
...
用户 ──HTTP──▶ API 层(FastAPI)
            ...
            Agent 核心(harness 管道)
              ...
              → Skill.finalize（确定性业务决策/写操作）→ guardrails 回复校验 → 状态机转移
                ▼ 只经 tool/service
       工具层(14工具,统一ToolResult,自动审计) · 服务层(order/logistics/refund/ticket/sla/knowledge) · LLM抽象层(provider可插拔)
                ▼
            PostgreSQL(11张表) + 可观测落库
```
...
## 数据库表（11）
users / orders / logistics / refund_requests / tickets / ticket_messages / agent_sessions / agent_tool_calls / knowledge_docs / quality_reviews(预留) / sla_records。

## API
- `GET /health`、`GET /`
- `POST /api/chat/message`、`GET /api/chat/session/{id}`
...
### 方式二：docker-compose（全栈：API + PostgreSQL + Redis + Celery worker）
```bash
...
## 后续优化方向
- QualityReviewSkill 会话质检评分；ComplaintDeEscalation / Invoice Skill
- 向量 RAG（现为关键词检索）；SSE/WebSocket 推送（现为轮询）
- 真实 cost 价目折算 + prompt caching 接入；SLA 超时监控任务
- MCP Server 封装；Alembic 迁移；前端两端（用户聊天端 / 运维监测端）对接
